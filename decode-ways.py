...
## if first digit 1 or 2 then consider double digit and less than 26
## add one to previous else not



class Solution:
  def decodeWays(self, string):
    ## base cases handle non digits
    memo = [0] * len(string)   
    ...
    for i in range(len(string)):
      ...
      if self.isValidNumber(string[i-1:i]):
        memo[i] = memo[i -1] + 1
      else:
        memo[i] = memo[i -1]
